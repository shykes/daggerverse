...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Utils":
		...
		case "CleanGoWorkspace":
			...
			return (*Utils).CleanGoWorkspace(&parent, source), nil
		case "GrepDir":
			var parent Utils
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Utils").
					WithFunction(
						dag.Function("CleanGoWorkspace",
							dag.TypeDef().WithObject("Directory")).
							WithDescription("Scan a Go workspace for modules called \"main\", and rename them to avoid conflicts\nThis fixes IDE auto-complete when developing \"daggerverse-style\" repositories with several modules").
